...
const ClientDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'inicio' | 'sesiones' | 'explorar' | 'perfil' | 'mensajes'>('inicio');
  ...
  const fetchSessions = async () => {
    ...
    const { data } = await supabase
      .from('sessions')
      .select(`*, advisors (*, profiles ( full_name )), services ( name, price, duration )`)
  ...
  const upcomingSessions = sessions.filter(s => s.status === 'confirmada' || s.status === 'pendiente');
  ...
  const getTabTitle = () => {
    ...
    if (activeTab === 'explorar') return 'Explorar asesores';
  ...
  return (
    <div className="flex h-screen bg-[#0A0E27] text-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <TopBar role="cliente" title={getTabTitle()} />
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {subscriptions.map((sub) => (
                      <GlassCard key={sub.id} className="p-5 border-[#10B981]/10">
                        <div className="flex items-center justify-between flex-wrap gap-4">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-[#10B981]/10 flex items-center justify-center text-[#10B981] font-black text-sm flex-shrink-0 overflow-hidden">
                            ...
                            <div>
                              <p className="text-white font-bold text-sm">
                                {sub.plan_type === 'plan_completo' ? 'Plan Completo' : 'Sesion Inicial'}
                              </p>
                              <p className="text-slate-500 text-[10px]">
                                {sub.advisors?.profiles?.full_name || 'Asesor'} — {sub.advisors?.category}
                              </p>
                              <p className="text-slate-600 text-[9px] mt-0.5">
                                Activo desde {new Date(sub.started_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                              </p>
                            </div>
                          </div>
                      ...
                      <GlassCard key={session.id} className="p-5 border-white/5">
                        <div className="flex items-center justify-between flex-wrap gap-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-white font-bold text-sm">{session.advisors?.profiles?.full_name || 'Asesor'}</p>
          ...
          {activeTab === 'sesiones' && (
            <div className="space-y-6">
              <h1 className="text-2xl font-light text-white uppercase tracking-tight">Mis sesiones</h1>
              {loadingSessions ? (
                <GlassCard className="p-8 border-white/5 text-center">
                  <p className="text-slate-500 text-sm animate-pulse">Cargando sesiones...</p>
                </GlassCard>
                ...
                <GlassCard className="p-12 border-white/5 text-center">
                  ...
                  <p className="text-slate-500 text-sm font-light mb-5">Aun no tienes sesiones registradas</p>
                  <Link
                    to="/asesores"
          ...
          {activeTab === 'explorar' && (
            <div className="space-y-6">
              <h1 className="text-2xl font-light text-white uppercase tracking-tight">Explorar asesores</h1>
              <GlassCard className="p-8 border-white/5 text-center">
                <Search size={32} className="text-[#10B981] mx-auto mb-4 opacity-50" />
                <p className="text-slate-400 text-sm mb-5 font-light">Encuentra el asesor perfecto para tu necesidad</p>
                <Link
                  to="/asesores"
                  className="inline-flex items-center gap-2 bg-[#10B981] text-[#0A0E27] font-black px-6 py-3 rounded-full text-[10px] uppercase tracking-wider hover:bg-[#0ea371] transition-all"
                >
