...
const Navbar = () => {
  ...
  // Páginas con fondo blanco → navbar claro
  const isLightPage =
    location.pathname === '/' ||
    location.pathname === '/como-funciona' ||
    location.pathname === '/nosotros' ||
    location.pathname.startsWith('/asesores');

  // Páginas con fondo oscuro → navbar siempre navy
  const isDark = !isLightPage;
  ...
  const navBg = isDark
    ? scrolled
      ? 'bg-[#0A0E27]/98 backdrop-blur-md shadow-lg shadow-[#0A0E27]/20'
      : 'bg-[#0A0E27]'
    : scrolled
      ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100'
      : 'bg-white';

  // Logo: en páginas claras siempre oscuro; en páginas oscuras siempre blanco
  const logoText   = isDark ? 'text-white'    : 'text-[#0A0E27]';
  const linkColor  = isDark
    ? 'text-white/70 hover:text-white hover:bg-white/10'
    : 'text-gray-500 hover:text-[#0A0E27] hover:bg-gray-50';
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center h-16 gap-8">
            ...
            >
              ...
              <div>
                <p className={`font-black tracking-tighter uppercase text-sm leading-none transition-colors duration-300 ${logoText}`}>
                  AXIOMA
                </p>
                <p className="text-[#10B981] text-[6px] font-bold tracking-[0.35em] uppercase leading-none mt-0.5">
              ...
              {user ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown
                      size={12}
                      className={`transition-transform ${userMenuOpen ? 'rotate-180' : ''} ${isDark ? 'text-white/40' : 'text-gray-400'}`}
                    />
                  </button>
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden z-50">
                      ...
                      <div className="p-1.5">
                        ...
                        >
                          <LayoutDashboard size={14} className="text-[#10B981]" /> Mi dashboard
                        </Link>
                        <Link
                          to={isAdvisor ? '/dashboard/asesor' : '/dashboard/cliente'}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-all"
                        >
                          <User size={14} className="text-[#10B981]" /> Mi perfil
                        </Link>
                        <button
          ...
          <div className={`md:hidden border-t px-6 py-4 space-y-1 ${
            ...
            <div className={`pt-2 border-t space-y-2 ${isDark ? 'border-white/10' : 'border-gray-100'}`}>
              {user ? (
                ...
                <>
                  <Link to="/login"
                    className={`block px-4 py-3 rounded-xl text-sm font-medium text-center border ${
                      isDark ? 'border-white/20 text-white/70' : 'border-gray-200 text-gray-600'
                    }`}>
                    Iniciar sesión
                  </Link>
                  <Link to="/registro"
                    className="block px-4 py-3 rounded-xl text-sm font-semibold text-center bg-[#10B981] text-white">
                    Comenzar
                  </Link>
                  <Link to="/registro?role=asesor"
                    className="block px-4 py-3 rounded-xl text-sm font-medium text-center text-[#10B981] border border-[#10B981]/30">
                    Ser asesor
                  </Link>
                </>
              )}
